...
void Sandbox2D::OnUpdate(Baku::Timestep ts)
{
	...
	{
		PROFILE_SCOPE("Renderer Begin Scene");
		Baku::Renderer2D::BeginScene(m_CameraController.GetCamera());
	}

	{
		PROFILE_SCOPE("Renderer Draw Quads");
        Baku::Renderer2D::DrawQuad({ -1.0f, 0.0f }, { 0.8f, 0.8f }, { 0.8f, 0.02f, 0.3f, 1.0f });
        Baku::Renderer2D::DrawQuad({ 0.5f, -0.5f }, { 0.5f, 0.75f }, { 0.2f, 0.03f, 0.8f, 1.0f });
        Baku::Renderer2D::DrawQuad({ 0.0f, 0.0f, -0.1f }, { 10.0f, 10.0f }, m_CheckerboardTexture);
    }

	{
		PROFILE_SCOPE("Renderer End Scene");
